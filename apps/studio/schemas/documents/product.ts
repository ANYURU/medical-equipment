...
export default defineType({
  ...
  fields: [
    ...
    { name: 'categories', title: 'Categories', type: 'array', of: [{ type: 'reference', to: [{ type: 'category' }] }], validation: (Rule) => Rule.required().min(1) },
    {
      name: 'specifications',
      ...
      of: [{
        ...
        preview: { select: { label: 'label', value: 'value' }, prepare: ({ label, value }) => ({ title: `${label}: ${value}` }) },
      }],
    },
    ...
    { name: 'status', title: 'Status', type: 'string', options: { list: ['available', 'coming_soon', 'discontinued'] }, initialValue: 'available' },
