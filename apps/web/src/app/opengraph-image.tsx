...
export default async function Image() {
  // Try to load the icon.svg as fallback
  let iconSvg: string | null = null
  try {
    const iconPath = join(process.cwd(), 'public', 'icon.svg')
    iconSvg = await readFile(iconPath, 'utf-8')
  } catch (error) {
    // Icon not found, use default design
  }
        ...
        {iconSvg ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '40px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              ...
              <div style={{ fontSize: '32px', opacity: 0.9, marginTop: '8px' }}>
                UGANDA LIMITED
              </div>
            </div>
